...
printf '# Work Log\n\nStarted dataset expansion for classics.\n' > work.md
...
async function main() {
  ...
  try {
    const response = await fetch(webhook, {
      ...
      body: JSON.stringify({ text: `[dataset-expansion] classics ${payload.checkpoint}: ${payload.summary}` }),
    });
...
2. Find and download one high-value public raw dataset that materially expands `classics`.
