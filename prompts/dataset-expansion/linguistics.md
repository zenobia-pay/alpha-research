...
printf '# Work Log\n\nStarted dataset expansion for linguistics.\n' > work.md
...
async function main() {
  ...
  try {
    const response = await fetch(webhook, {
      ...
      body: JSON.stringify({ text: `[dataset-expansion] linguistics ${payload.checkpoint}: ${payload.summary}` }),
    });
...
2. Find and download one high-value public raw dataset that materially expands `linguistics`.
