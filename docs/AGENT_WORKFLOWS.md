...
For small public-source additions that need code execution on the mounted volume, prefer a direct `modal run` script. `scripts/modal-add-econ-eia-seds.py` is the first reusable example: it mounts `agent-dataset`, writes under `/datasets/econ/raw/`, updates `manifest.json`, commits the Modal volume, and returns compact evidence for briefing/profile sync.
