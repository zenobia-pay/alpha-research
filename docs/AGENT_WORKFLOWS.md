...
For small public-source additions that need code execution on the mounted volume, prefer a direct `modal run` script. `scripts/modal-add-econ-eia-seds.py`, `scripts/modal-add-econ-eia-open-data-bulk.py`, and `scripts/modal-add-econ-bls-laus-bulk.py` are reusable examples: they mount `agent-dataset`, write under `/datasets/econ/raw/`, update `manifest.json`, commit the Modal volume, and return compact evidence for briefing/profile sync.
