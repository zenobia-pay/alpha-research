...
The harness starts the canonical `econ` public environment and captures the resource contract sent to the backend.

## Assertions Made

The request uses the `canonical-public` Modal profile, a 50GiB scratch workspace, object-store versioning, read-only dataset-version access, and versioned publish mode.
