...
- A 2026-06-01 inode-hotspot audit found the prior inode pressure was dominated by `.remote-agent` automation cache/workspace entries, not provider-native `raw/` economics data. Cleanup run `2d9fe31b-2951-4c52-afe6-f2c5f21f78cb` removed stale cache targets and recovered about 90,249 free inodes. Resume provider downloads as small, controlled additions with `df -i` monitoring after each run; prefer compact provider-native archives and avoid unnecessary extraction into many small files.
