...
```text
Narrow canonical econ improvement: add official U.S. Census Building Permits Survey Master Data Set provider-native files. The prior execution b1d5b358-0368-4edc-9d66-5c7269cac6b7 stalled after scratch-path and malformed command issues; avoid that pattern.

Use the mounted dataset root directly. Do not write the source files to worker-local ./raw first. Create and use exactly this target directory on the mounted dataset volume:
/data/datasets/econ/raw/census_bps_master_202604/

Before downloading, prove writability with:
probe=/data/datasets/econ/.canonical_bps_probe_<run_id>; printf ok > "$probe" && rm "$probe"

Use only official Census public URLs:
- https://www2.census.gov/econ/bps/Master%20Data%20Set/BPS_Compiled_File_202604.zip
- https://www2.census.gov/econ/bps/Master%20Data%20Set/Compiled%20Data%20Documentation.docx
- https://www2.census.gov/econ/bps/Master%20Data%20Set/Compiled%20File%20Sample.csv

Download with simple direct commands, for example curl -L --fail --retry 3 --output /data/datasets/econ/raw/census_bps_master_202604/<filename> <url>. Preserve provider-native files exactly. Do not expand the 458 MB ZIP into many files on disk. Do not create merged panels, derived fields, or model-ready tables.

Required evidence to collect in artifacts and briefing:
- For all three files: final URL, HTTP status/header evidence where feasible, byte size, SHA-256, access timestamp.
- For BPS_Compiled_File_202604.zip: unzip -t integrity result, zipinfo member inventory, compressed/uncompressed member sizes, and cheap row/header counts for contained CSV members using streaming from unzip -p if feasible.
- For the sample CSV: header and row count.
- For the DOCX: byte/hash/provenance only is enough.

After confirming the files exist under /data/datasets/econ/raw/census_bps_master_202604/, update the full public briefing, MDX mirror, manifest/profile/quality metadata, and artifacts. Preserve the accumulated econ inventory exactly; add a new literal inventory bullet for raw/census_bps_master_202604/ and update the housing roadmap so building permits are present in part, not complete. Do not claim all economics data is complete.

Final status should support npm run canonical:dataset -- validate --dataset-id econ --execution-id <id> if possible.
```
