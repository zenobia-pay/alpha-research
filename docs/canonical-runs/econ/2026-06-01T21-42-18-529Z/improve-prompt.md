...
Objective: close the documented firm/industry roadmap gap for Census ABS/ASE owner-demographic business data by adding one compact, official, public Census source package to the mounted econ canonical dataset.
...
- Prefer the latest official Census Annual Business Survey / Annual Survey of Entrepreneurs / Nonemployer Statistics public downloadable package that covers business owner demographics, firm characteristics, receipts, employment/payroll, industry, geography, race/ethnicity/sex/veteran ownership, or related entrepreneurial measures.
- Store provider-native ZIP/CSV/XLSX/TXT files under a specific raw directory such as `raw/census_abs_ase_owner_demographics_<year>/` or a clearer provider/year-specific name.
- Avoid expanding huge archives into many small files unless required for validation; preserve compressed provider files when possible.
- Capture HTTP status, final URLs, content-length/last-modified/etag where available, SHA-256 hashes, byte counts, ZIP integrity/member inventories when applicable, row/column counts from streamed inspection where feasible, retrieval timestamps, and provider citation notes.
- If the preferred ABS/ASE owner-demographic source is unavailable or blocked, classify candidates and select the best adjacent official Census nonemployer/business-demographics public package. If no suitable public package is downloadable, do not write placeholder data; report the blocker clearly.

Required artifacts:
- `dataset_briefing.md` with a literal inventory bullet for the new raw directory and explicit remaining gaps.
- `improvement_result.json` containing source URLs, files, hashes, byte counts, row counts where feasible, target directory, and validation/provenance notes.
- `metadata.json` and/or `inventory.csv` in the raw directory.
- Updated canonical profile/readback tied to this execution id.

After writing, verify the raw files exist on disk, run lightweight integrity checks, sync the profile briefing, and report whether the dataset remains disk-proven.
```
