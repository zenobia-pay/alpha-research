...
Objective: close part of the documented firm, industry, and labor-market roadmap gap by adding a compact, official, public Bureau of Labor Statistics Business Employment Dynamics (BED) source package to the mounted econ canonical dataset.
...
- Prefer official BLS BED public downloadable files covering establishment births/deaths, gross job gains/losses, firm age/size, industry, state, or national time-series. Pick one focused source family that can complete in one run.
- Store provider-native CSV/TXT/XLSX/ZIP files under a specific raw directory such as `raw/bls_bed_<source_family>_<date>/`.
