# Econ BIS GDP Read-Only Inspection

You are running an admin-owned canonical maintenance diagnostic for `econ`.

Objective: inspect the already-present `raw/bis_gdp` directory on the canonical econ dataset volume and produce evidence artifacts that can support a conservative documentation/profile update. This is a read-only inspection, not a download or dataset-writing job.
...
3. Identify the likely provider/source from file names and metadata. If the files prove Bank for International Settlements GDP/reference macro data, say so; otherwise classify the source as unknown and explain why.
4. Record what the source adds to economist coverage and what it does not cover.
5. Write `bis_gdp_inventory.json` with structured fields:
   - `status`
   - `datasetDir`
   - `sourceDir`
   - `directoryExists`
   ...
   - `coverageSummary`
   - `limitations`
6. Write `bis_gdp_inventory.md` as a compact human-readable summary.
