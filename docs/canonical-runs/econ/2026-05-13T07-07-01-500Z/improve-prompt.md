...
```text
Refresh the canonical public dataset briefing from the mounted dataset volume. Inventory the on-disk dataset contents, summarize key tables/fields and coverage, and write the final briefing to dataset_briefing.md on the mounted dataset volume.
```
