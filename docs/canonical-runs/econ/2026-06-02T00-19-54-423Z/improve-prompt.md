...
```text
Canonical admin improvement job for dataset `econ`: add one compact, high-value official public macro source package that directly reduces the documented ONS regional-accounts gap.
...
- Target only official UK Office for National Statistics public source files for regional accounts. Prefer one current provider-native downloadable workbook or time-series package such as regional gross value added, regional gross disposable household income, regional accounts, or regional economic activity tables. Do not use unofficial mirrors.
- Keep the job focused. Add one coherent ONS regional-accounts package or a small set of directly companion files. Avoid broad ONS catalog crawling.

Storage and data rules:
- Store provider-native raw files under a new stable raw directory such as `raw/ons_regional_accounts_<release-or-capture-date>/`.
...
- For ZIP/XLS/XLSX/CSV files, inspect enough structure to report member/sheet names, row/column counts, period coverage, geography coverage, key economic concepts, and units where visible.
- Record whether files are provider-native and whether any transformations were avoided.

Required artifacts:
- `dataset_briefing.md`: a full updated dataset inventory briefing that preserves all prior inventory bullets and adds the new ONS regional-accounts entry near the top.
...
- Preserve the roadmap limitation language. Mark ONS regional accounts as present in part only if a real package lands; do not claim full ONS regional coverage, Blue Book/Pink Book revision histories, or full ONS API/catalog completion.
