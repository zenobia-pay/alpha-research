...
```text
Repair and validate the BEA international transactions/detail data apparently written by failed expansions 53f9c915-ebab-48a1-8443-d6162773c2fe and c6b2dd6f-0d1b-4d96-9ac5-60d21d0522a1. Inspect the mounted econ volume for raw/bea_itd or BEA_ITD_CountryDetail files, verify provider-native file integrity and coverage, update inventories and dataset_briefing.md only for data actually present on disk, sync the backend profile to this current execution id, and emit all required artifacts even if blocked.
```
