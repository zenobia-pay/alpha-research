...
```text
Focused Econ briefing/profile repair execution test. Execute now; do not stop after planning. Do not perform broad source expansion. Read current mounted inventories and dataset_briefing.md, preserve raw holdings, regenerate only if stale, ensure dataset_briefing.md, docs mirrors, and CLI briefingMarkdown are a comprehensive # Literal Data Inventory. Use update_remote_dataset_profile tool/function if available; do not rely on shell curl or localhost for profile update. Copy dataset_briefing.md, docs/public-datasets/briefings/econ.md, docs/public-datasets/econ.mdx, improvement_result.json, and volume_inventory_summary.json into the remote run artifact directory. Verify readback contains # Literal Data Inventory and this run id.
```
