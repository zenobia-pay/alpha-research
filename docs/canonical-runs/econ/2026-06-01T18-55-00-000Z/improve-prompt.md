...
```text
Focused canonical econ improvement: add the official Federal Housing Finance Agency House Price Index master CSV to narrow the housing and real-estate roadmap gap.

Use only official FHFA public sources. Verify the FHFA HPI datasets page and the direct master CSV URL before writing data:
- Landing page: https://www.fhfa.gov/house-price-index?tab=HPI+Datasets
- Direct CSV candidate: https://www.fhfa.gov/hpi/download/monthly/hpi_master.csv

If the direct URL does not return HTTP 200 text/csv, derive the current official Master HPI Data CSV href from the FHFA page and record that evidence. Do not use mirrors or unofficial copies.

Write dataset data only under `raw/fhfa_hpi_master_20260601/`. Preserve the provider-native CSV as downloaded, preferably as `hpi_master.csv`; do not create merged panels, derived fields, cross-source joins, or analysis-ready artifacts. It is okay to create compact metadata/inventory files in the same directory.

Required evidence and inventory for the new FHFA file:
- source URL and final URL;
- HTTP status, content type, content length, cache/last-modified/etag headers when available;
- byte size and SHA-256 hash;
- CSV header, row count, column count, time-period coverage, geography levels, index/frequency/flavor fields, and a concise coverage summary.

Update the full mounted dataset inventory, `dataset_briefing.md`, docs mirrors, manifest/profile/quality metadata, and produced artifacts while preserving the existing accumulated econ inventory. The final briefing and profile must not collapse the prior inventory to only this FHFA addition. Do not claim the econ dataset now literally has all data an economist could need; state that FHFA HPI is one more concrete housing/real-estate gap narrowed and keep remaining roadmap gaps.

Final status must be suitable for local validation with `npm run canonical:dataset -- validate --dataset-id econ --execution-id <execution-id>`: required runtime artifacts present, `profile.quality.diskInventoryProven` true, and profile readback tied to this execution id.
```
