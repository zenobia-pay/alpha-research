...
Objective: close part of the documented UK/ONS macro gap by adding one compact, official, public Office for National Statistics prices or labour-market source package to the mounted econ canonical dataset.
...
- Prefer official ONS downloadable/public API files for CPIH/CPI detailed tables, monthly GDP, labour-market releases, trade, productivity, or balance-of-payments/IIP. Pick one focused source family that can complete in one run, ideally CPIH/CPI or labour-market time series.
- Store provider-native CSV/XLSX/TXT/ZIP/JSON files under a specific raw directory such as `raw/ons_<source_family>_<date>/`.
- Avoid expanding huge archives into many small files unless required for validation; preserve provider files when possible.
- Capture HTTP status, final URLs, content-length/last-modified/etag where available, SHA-256 hashes, byte counts, workbook/sheet or table inventories, row/column counts from streamed inspection where feasible, retrieval timestamps, and provider citation/license notes.
