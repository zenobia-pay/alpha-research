...
async function main() {
  ...
  try {
    const response = await fetch(webhook, {
      ...
      body: JSON.stringify({ text: `[dataset-expansion] econ ${payload.checkpoint}: ${payload.summary}` }),
    });
...
2. Find and download one high-value public raw economics dataset that materially expands `econ`. Prefer broad, authoritative economics data over small samples or blocked/partial attempts.
