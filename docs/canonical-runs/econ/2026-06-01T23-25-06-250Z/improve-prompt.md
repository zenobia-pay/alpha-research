...
Target gap: add a compact provider-native UK Office for National Statistics trade source package to the raw economics substrate. Existing ONS coverage has quarterly national accounts, monthly GDP, CPIH/CPI reference tables, and a labour-market overview workbook; ONS trade remains a named roadmap gap.
...
- Prefer official ONS endpoints and current ONS dataset file/API URLs for UK trade in goods/services, monthly trade, trade by commodity/country, or UK trade bulletin reference tables. Capture a compact provider-native workbook/CSV/zip package that preserves release tables and metadata.
...
- Classify as a public-source macro/international-trade improvement.
- Do not claim this completes ONS coverage. Explicitly preserve remaining gaps such as ONS productivity, balance of payments/IIP, regional accounts, Blue Book revision histories, broader ONS API/catalog coverage, detailed trade microdata, and all historical trade vintages.
```
