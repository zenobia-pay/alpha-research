...
Target gap: add a compact provider-native UK Office for National Statistics productivity source package to the raw economics substrate. Existing ONS coverage has quarterly national accounts, monthly GDP, CPIH/CPI reference tables, labour-market overview, and UK trade publication tables; ONS productivity remains a named roadmap gap.

Constraints:
- Treat this as an admin-owned canonical dataset improvement job, not a user-facing research run.
- Use the mounted canonical dataset volume from `DATASET_MOUNT_PATH` when available.
- Before downloading, prove the dataset directory is writable with an actual create/delete probe in the dataset root. If the probe fails, stop and report `dataset_dir_not_writable` with the non-secret filesystem error.
- Preserve raw public source files only. Do not create merged panels, derived fields, cross-source joins, imputed data, or analysis-ready artifacts.
- Prefer official ONS endpoints and current ONS dataset file/API URLs for productivity overview, labour productivity, output per hour, output per worker, productivity jobs/hours, industry productivity, or UK productivity reference tables. Capture a compact provider-native workbook/CSV/zip package that preserves release tables and metadata.
- Keep the addition narrow enough to avoid inode pressure. Store source files provider-native; do not expand large archives into many small files unless required for inventory evidence.
- Include local metadata with source URLs, retrieval timestamp, HTTP status, byte counts, SHA-256 hashes, content type, provider/license notes, and any Last-Modified/ETag headers.
- Add an inventory file for the captured package and inspect enough workbook/CSV structure to state sheet/table names, row/column counts, and coverage dates.
- Update the canonical econ briefing and docs mirrors to describe exactly what was added and what remains missing.
- Update the dataset profile only after readback proves the profile points at this execution id and the briefing is not a startup placeholder.
- Required final artifacts: `dataset_briefing.md`, `improvement_result.json`, and a concise completion note with the raw directory path, source URLs, hashes, coverage dates, validation/readback evidence, and limitations.

Candidate classification:
- Classify as a public-source macro/productivity improvement.
- Do not claim this completes ONS coverage. Explicitly preserve remaining gaps such as ONS balance of payments/IIP, regional accounts, Blue Book revision histories, broader ONS API/catalog coverage, detailed productivity microdata, and all historical productivity vintages.
```
