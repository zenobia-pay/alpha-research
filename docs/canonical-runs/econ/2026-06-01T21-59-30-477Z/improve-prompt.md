...
Objective: close part of the documented macro/international roadmap gap for ONS bulk data by adding one compact, official, public UK Office for National Statistics source package to the mounted econ canonical dataset.
...
- Prefer official ONS public bulk downloads/API packages that materially improve macroeconomic coverage: UK national accounts, CPI/inflation, labor market, population, trade, public sector finance, productivity, or regional economic series. Pick a focused source family that can complete in one run.
- Store provider-native CSV/ZIP/XLSX/JSON files under a specific raw directory such as `raw/ons_<source_family>_<date>/`.
...
- Capture HTTP status, final URLs, content-length/last-modified/etag where available, SHA-256 hashes, byte counts, ZIP/workbook/member inventories when applicable, row/column counts from streamed inspection where feasible, retrieval timestamps, and provider citation/license notes.
- If the preferred ONS source is unavailable or blocked, classify candidates and select the best adjacent official ONS public macro/labor package. If no suitable public package is downloadable, do not write placeholder data; report the blocker clearly.
