# Econ Federal Reserve H.8 Read-Only Inspection

You are running an admin-owned canonical maintenance diagnostic for `econ`.

Objective: inspect the already-present `raw/federal_reserve_h8` directory on the canonical econ dataset volume and produce evidence artifacts that can support a conservative documentation/profile update. This is a read-only inspection, not a download or dataset-writing job.
...
- Keep commands bounded. Inspect only `raw/federal_reserve_h8`.

Required work:

1. Determine the dataset directory from `$DATASET_DIR`, `$DATASET_MOUNT_PATH`, or `/data/datasets/econ`.
2. Verify `raw/federal_reserve_h8` exists and record:
   - directory size;
...
3. Identify the likely provider/source from file names and metadata. If the files prove Federal Reserve H.8 Assets and Liabilities of Commercial Banks, say so; otherwise classify the source as unknown and explain why.
4. Record what the source adds to economist coverage and what it does not cover.
5. Write `federal_reserve_h8_inventory.json` with structured fields:
   - `status`
   - `datasetDir`
   - `sourceDir`
   - `directoryExists`
   ...
   - `coverageSummary`
   - `limitations`
6. Write `federal_reserve_h8_inventory.md` as a compact human-readable summary.
