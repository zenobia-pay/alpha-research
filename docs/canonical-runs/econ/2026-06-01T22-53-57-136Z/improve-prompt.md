...
Target gap: add a compact provider-native UK Office for National Statistics monthly GDP / monthly GDP estimate source package to the raw economics substrate. This should improve the existing ONS coverage, which currently has quarterly national accounts and CPIH/CPI detailed reference tables but still lacks monthly GDP coverage.
...
- Prefer official ONS endpoints and current ONS dataset file/API URLs for monthly GDP or monthly GDP estimate releases. If there are multiple official formats, capture the most compact provider-native workbook/CSV/zip files that preserve the monthly GDP estimate tables and metadata.
...
- Required final artifacts: `dataset_briefing.md`, `improvement_result.json`, and a concise completion note with the raw directory path, source URLs, hashes, coverage dates, validation/readback evidence, and any limitations.

Candidate classification:
- Classify as a public-source macro/vintage improvement.
- Do not claim this completes ONS coverage. Explicitly preserve remaining gaps such as ONS labour market, trade, productivity, balance of payments/IIP, regional accounts, Blue Book revision histories, and broader ONS API/catalog coverage.
```
