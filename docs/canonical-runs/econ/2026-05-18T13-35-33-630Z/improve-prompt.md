...
```text
Refresh the public dataset briefing from the mounted canonical dataset volume. Inventory the on-disk dataset contents, verify that the briefing describes available records and useful fields, and write the final source-of-truth briefing to dataset_briefing.md on the mounted dataset volume.
```
...
3. Use the mounted dataset volume as the dataset root. Prefer `DATASET_MOUNT_PATH`; otherwise use `/mnt/alpha-research/datasets/econ`.
...
7. Copy the exact same briefing body into `docs/public-datasets/briefings/econ.md` and `docs/public-datasets/econ.mdx` in the run artifact/workspace area when available.
...
10. Copy `dataset_briefing.md`, `docs/public-datasets/briefings/econ.md`, `docs/public-datasets/econ.mdx`, `improvement_result.json`, `volume_inventory_summary.json`, `work.md`, and `report.html` into the remote run artifact directory so the orchestrator can recover them.
