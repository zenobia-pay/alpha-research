...
```text
Focused canonical econ improvement: add official U.S. Census Building Permits Survey Master Data Set compiled file to narrow the housing/real-estate building-permits gap.

Use only official Census public sources. Verify https://www2.census.gov/econ/bps/Master%20Data%20Set/ and these direct URLs before writing:
...
Download and preserve provider-native files under raw/census_bps_master_202604/:
- BPS_Compiled_File_202604.zip
- Compiled Data Documentation.docx
- Compiled File Sample.csv

Preserve provider-native archive/docs/sample; do not create merged panels, derived fields, cross-source joins, or model-ready tables.

For the ZIP, record HTTP status/headers, final URL, byte size, SHA-256, ZIP integrity, member inventory, compressed/uncompressed sizes, and cheap streamed CSV row/column counts for contained members where feasible without extracting full data to disk.

For DOCX/sample, record byte size, SHA-256, content type, last-modified, and sample row/header evidence.

Update the full mounted dataset inventory, briefing, MDX mirror, manifest/profile/quality metadata, and artifacts. Preserve the accumulated econ inventory and do not claim all economics data is complete.
