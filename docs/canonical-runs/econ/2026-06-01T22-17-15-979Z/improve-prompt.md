...
Objective: close part of the documented public finance, tax, and transfer-system roadmap gap for SSA/OASDI public data by adding one compact, official, public Social Security Administration source package to the mounted econ canonical dataset.
...
- Prefer official SSA/OASDI public data that materially improves economics coverage: Annual Statistical Supplement tables, OASDI monthly/annual benefit and beneficiary tables, trust fund/actuarial public data, SSI annual statistical data, or downloadable SSA research/statistical files. Pick one focused source family that can complete in one run.
- Store provider-native CSV/XLSX/PDF/TXT/ZIP/HTML files under a specific raw directory such as `raw/ssa_oasdi_<source_family>_<date>/`.
- Avoid expanding huge archives into many small files unless required for validation; preserve provider files when possible.
- Capture HTTP status, final URLs, content-length/last-modified/etag where available, SHA-256 hashes, byte counts, workbook/sheet or table inventories when applicable, row/column counts from streamed inspection where feasible, retrieval timestamps, and provider citation/license notes.
- If the preferred SSA/OASDI source is unavailable or blocked, classify candidates and select the best adjacent official SSA public transfer-system package. If no suitable public package is downloadable, do not write placeholder data; report the blocker clearly.
