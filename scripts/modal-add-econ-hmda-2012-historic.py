"""Direct Modal-volume addition for CFPB HMDA 2012 historic ZIPs."""
...
APP_NAME = "econ-direct-hmda-2012-historic"
DATASET_ROOT = Path("/data/datasets/econ")
RAW_DIR = DATASET_ROOT / "raw" / "hmda_2012_historic"
REFERER = "https://www.consumerfinance.gov/data-research/hmda/historic-data/"
FILES = [
    {
        "filename": "hmda_2012_nationwide_all-records_labels.zip",
        "url": "https://files.consumerfinance.gov/hmda-historic-loan-data/hmda_2012_nationwide_all-records_labels.zip",
        "source_key": "hmda_2012_historic_loan_records",
    },
    {
        "filename": "hmda_2012_transmittal_sheet.zip",
        "url": "https://files.consumerfinance.gov/hmda-historic-institution-data/hmda_2012_transmittal_sheet.zip",
        "source_key": "hmda_2012_historic_transmittal_sheet",
    },
    {
        "filename": "hmda_2012_panel.zip",
        "url": "https://files.consumerfinance.gov/hmda-historic-institution-data/hmda_2012_panel.zip",
        "source_key": "hmda_2012_historic_panel",
    },
]
...
def add_hmda_2012_historic() -> dict[str, object]:
    ...
    metadata = {
        "dataset_id": "econ",
        "source_key": "hmda_2012_historic",
        "title": "CFPB HMDA 2012 Historic Loan and Institution Data",
        "retrieved_at": retrieved_at,
        "raw_dir": str(RAW_DIR.relative_to(DATASET_ROOT)),
        "records": records,
        "notes": [
            "Provider-native official CFPB historic ZIP files retained compressed.",
            "Loan records are the CFPB historic nationwide all-records archive, not the modern full snapshot LAR format.",
...
def main() -> None:
    print(json.dumps(add_hmda_2012_historic.remote(), indent=2, sort_keys=True))
