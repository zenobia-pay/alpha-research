...
test("dataset expansion prompt uses explicit dataset and artifact directories", () => {
  ...
  for (const required of [
    ...
    "If any step after startup blocks, times out, or fails, do not end the run without the required artifacts.",
    "Copy the current `$DATASET_DIR/dataset_briefing.md` to `./dataset_briefing.md` and `$ARTIFACT_DIR/dataset_briefing.md` when it exists",
    "Write `improvement_result.json` with `\"status\": \"blocked\"`",
    "CANONICAL_DATASET_SLACK_WEBHOOK_URL",
